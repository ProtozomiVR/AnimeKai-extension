...
class AnimeKaiExtractors(
    ...
    /**
     * The user-facing label applied to every extracted Video. We suffix it
     * with the quality tag when one is available so the user can tell the
     * variants apart in the player UI.
     */
    private fun Video.copy(tag: String): Video = Video(
        url = this.url,
        quality = tag,
    ...
    private fun extractStreamWish(embedUrl: String, tag: String): List<Video> {
        ...
        val videos = pattern.findAll(html).map { m ->
        ...
        if (videos.isEmpty()) {
            ...
            runCatching {
                ...
                Regex(""""file"\s*:\s*"(https?://[^"]+)"""").findAll(body)
                    .forEach { m ->
                        val u = m.groupValues[1]
                        if (u.endsWith(".m3u8") || u.endsWith(".mp4")) {
                            videos.plusAssign(Video(u, "$tag - Default", u,
                                headers = baseHeaders.newBuilder().set("Referer", embedUrl).build()))
                        }
                    }
            }
